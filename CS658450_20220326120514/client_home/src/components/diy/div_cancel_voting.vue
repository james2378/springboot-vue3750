<template>
	<div class="diy_details_box diy_div_cancel_voting">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "cancel_voting_id",
					source_id: this.obj["cancel_voting_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/cancel_voting/set?cancel_voting_id=' + _this.obj["cancel_voting_id"], {
								praise_len
							}, (res) => {
								if(res.result){
