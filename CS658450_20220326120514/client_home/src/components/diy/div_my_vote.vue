<template>
	<div class="diy_details_box diy_div_my_vote">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "my_vote_id",
					source_id: this.obj["my_vote_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/my_vote/set?my_vote_id=' + _this.obj["my_vote_id"], {
								praise_len
							}, (res) => {
								if(res.result){
