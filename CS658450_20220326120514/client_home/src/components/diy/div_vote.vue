<template>
	<div class="diy_details_box diy_div_vote">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/my_vote/edit','add')" @click="to_form('/my_vote/edit')" >投票 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/vote/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "vote",
					source_field: "vote_id",
					source_id: this.obj["vote_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/vote/set?vote_id=' + _this.obj["vote_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.voting_topic
					query.img = this.obj.voting_cover
