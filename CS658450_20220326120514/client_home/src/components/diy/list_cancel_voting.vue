<template>
	<div class="diy_home diy_list diy_cancel_voting" id="diy_cancel_voting_list">
		<!-- 列表 -->
		<div class="diy_view_list list list-x" v-if="show">
			<router-link class="diy_card goods diy_list_box_wrap" v-for="(o, i) in list" :key="i"
				:to="'/cancel_voting/details?cancel_voting_id=' + o['cancel_voting_id']">
				<!-- 图片 -->
				<div class="diy_list_img_box" v-if="imgList.length" >
					<div class="diy_row" v-for="(item,index) in imgList" :key="item+index" v-show="$check_field('get',item.name,'/cancel_voting/details') && +item.is_img_list">
				...
				<div class="diy_list_item_box">
					<div class="diy_list_item_content" v-for="(item,index) in showItemList" :key="item+index">
						<div class="diy_row" :class="{[item.name]:true}" v-if="$check_field('get',item.name,'/cancel_voting/details') && +item.is_img_list">
							<div class="diy_title">
								<span>{{item.title}}:</span>
							</div>
		...
		<div class="diy_view_table" v-else>
			<table class="diy_table">
				<tr class="diy_row">
					...
					<th class="diy_title" v-if="$check_field('get','number_of_cancelled_votes')">取消票数
					</th>
				...
				<tr class="diy_row" v-for="(o,i) in list" :key="o+i">
					...
					<td class="diy_field diy_text" v-if="$check_field('get','number_of_cancelled_votes')">
						<span>
							{{ o["number_of_cancelled_votes"] }}
						</span>
					</td>
		...
		data() {
			return {
						...
						itemList: [
								...
								{
									title: "取消票数",
									name: "number_of_cancelled_votes",
									type: "下拉",
									is_img_list: "0"
								},
						],
