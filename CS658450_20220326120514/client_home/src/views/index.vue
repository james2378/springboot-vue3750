		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/vote/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_vote = !show_list_vote">
		                        <div class="switch_box">
		                            <span v-if="show_list_vote"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--参与投票推荐列表组件 -->
		                <list_vote v-if="show_list_vote" :list="list_vote" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_vote.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_vote" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_vote" :key="i" @click="to_details('vote',o,'vote_id')">
		                            <td v-for="(oj,n) in fields_vote">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_vote from "@/components/diy/list_vote.vue";
	...
	export default {
		...
		methods: {
			...
			// 获取参与投票列表
			get_vote() {
				let url = "~/api/vote/get_list?";
				url = "~/api/vote/get_list?orderby=hits desc";
				this.$get(url, {
					...
					if (json.result) {
						this.list_vote = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_vote() {
		        var list = this.list_vote;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_vote.map((o) => {
		                // 第二个中括号是对象的属性
